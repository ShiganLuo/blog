...
- ​**文章列表**：
    - 分页加载或无限滚动。
    - 显示文章标题、摘要、发布时间、作者、分类、标签、阅读量、点赞数、评论数。
    - 支持按分类、标签、时间范围筛选文章。
- ​**文章详情**：
    - 文章内容展示（支持 Markdown 和富文本格式）。
    - 文章目录（根据标题生成）。
    - 上一篇、下一篇导航。
    - 相关文章推荐（基于分类或标签）。
    - 分享功能（分享到社交媒体）。
...
- 每篇文章的 SEO 设置（标题、描述、关键词）。
- 自动生成 sitemap.xml 和 robots.txt。
- 友好的 URL 结构（如 `/category/technology`）。

#### ​第三方集成

- 社交媒体分享按钮（如 Twitter、Facebook、WeChat）。
- 第三方评论系统（如 Disqus、Gitalk）。
- 第三方登录（如 GitHub、Google、微信）。

#### ​数据统计

- 文章阅读量、点赞数、评论数统计。
- 用户访问量、访问来源（设备、浏览器、地区）。
## 博客后台
...
- ​**用户注册与登录**：
    - 支持邮箱/手机号注册和登录。
    - 支持第三方登录（如 GitHub、Google、微信）。
    - 验证码、密码强度校验。
- ​**用户角色与权限**：
    - 角色划分：管理员、编辑、普通用户。
    - 权限控制：基于角色的功能访问限制（如管理员可删除文章，编辑可发布文章）。
- ​**用户信息管理**：
    - 修改个人信息（头像、昵称、简介）。
    - 修改密码、绑定邮箱/手机号。
- ​**用户行为管理**：
...
- ​**站点设置**：
    - 站点基本信息（名称、Logo、描述）。
    - 站点 SEO 设置（全局标题、描述、关键词）。
    - 站点统计代码（如 Google Analytics）。
- ​**邮件通知**：
    - 新评论通知、用户注册通知。
    - 邮件模板管理。
- ​**日志管理**：
...
- 提供 RESTful API，支持第三方应用接入。
- 接口权限控制（API Key、OAuth）。
...
`NOT NULL` 约束解决的是“能不能不写值”，  
`DEFAULT` 解决的是“没写值时，数据库替你写什么”。

| 结果           | 设计                     |
| ------------ | ---------------------- |
| 有默认值 + 不允许未知 | `NOT NULL DEFAULT xxx` |
| 无默认值 + 不允许未知 | `NOT NULL`             |
| 允许未知         | `NULL`（慎用）             |
...
| Java 类型        | MySQL `BIGINT` 是否兼容 | 适用场景                     |
| -------------- | ------------------- | ------------------------ |
| `int`          | ✅ 兼容（自动转换）          | **小数值**（不超过 `2^31 - 1`）  |
| `long`         | ✅ 直接匹配              | **推荐**，适用于所有 `BIGINT` 值  |
| `Integer`（包装类） | ✅ 兼容                | 用于可 `NULL` 值的 `BIGINT`   |
| `Long`（包装类）    | ✅ 直接匹配              | **推荐**，适用于 JPA/Hibernate |
