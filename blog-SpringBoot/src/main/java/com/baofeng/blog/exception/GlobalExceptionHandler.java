...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<?>> handleJsonMappingException(JsonMappingException ex) {
        ...
        if (cause instanceof IllegalArgumentException) {
            return ResponseEntity.badRequest().body(ApiResponse.error(400, cause.getMessage()));
        }
        return ResponseEntity.badRequest().body(ApiResponse.error(400, "请求参数格式错误"));
    }
    ...
    public ResponseEntity<ApiResponse<?>> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
        ...
        if (cause != null && cause.getCause() instanceof IllegalArgumentException iae) {
            // 捕获 record 构造器抛的异常
            return ResponseEntity.badRequest().body(ApiResponse.error(400, iae.getMessage()));
        }
        return ResponseEntity.badRequest().body(ApiResponse.error(400, "请求参数格式错误"));
    }
    //自定义重复异常
