...
public record ApiResponse<T>(int code, String message, T result) {
    ...
    public static <T> ApiResponse<T> success(T result) {
        return new ApiResponse<>(ResultCode.SUCCESS.code(), ResultCode.SUCCESS.message(), result);
    }
     ...
     *
     * @param code 错误码枚举对象 {@link ResultCode}
     ...
     * @return 错误响应对象
     */
    public static <T> ApiResponse<T> error(ResultCode code, String customMessage) {
        return new ApiResponse<>(code.code(), customMessage, null);
    }

    /**
     * 返回一个通用失败响应，使用自定义状态码和消息。
     * <p>适用于没有定义在 {@link ResultCode} 中的错误场景。</p>
     *
     * @param code    自定义状态码
     * @param message 自定义消息
     * @param <T>     数据类型（通常为 null）
